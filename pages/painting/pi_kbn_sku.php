  ...
  <script>
    ...
    function renderPaginatedTable(pageData) {
      ...
      pageData.forEach(item => {
        if (item.assembly_section !== 'PAINTING') return; // only show assembly section
        ...
        row.innerHTML = `
            <td style="text-align: center;" class="sticky-col col-1">
                <span style="cursor: pointer; margin-right: 12px;" onclick="handleRefresh('${item.id}','painting','${item.assembly_section}','assembly','assembly')">🔄</span>
            </td>
      ...
      fetch('api/assembly/timeinOperator', {
        ...
        .then(data => {
          if (data.success) {
            ...
            setTimeout(() => window.location.reload(), 2000); // reload after auto-close
          } else {
    ...
    document.getElementById('quantityForm').addEventListener('submit', function(e) {
      ...
      if (totalIfSubmitted > maxQuantity) {
        showAlert(
          'warning',
          'Exceeded Quantity',
          `The total quantity being assembled for Reference No: ${currentRef}, section ${currentSection} and process #${currentProcessNo} exceeds the allowed maximum.\n\n` +

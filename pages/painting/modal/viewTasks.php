...
<script>
    ...
    function renderTasks(user_id, full_name, selectedItem = null) {
        ...
        fetch(
                'api/assembly/getAllModelData_assigned?model=' + encodeURIComponent(model || 'L300 DIRECT') + '&_=' + new Date().getTime(), {
                    method: 'POST',
                    headers: {
            ...
            .then(data => {
                ...
                // Ensure selectedItem is always included
                if (selectedItem && selectedItem.id && selectedItem.material_no && selectedItem.material_description) {
                ...
                items.forEach(item => {
                    ...
                    if (!modelsMap[m]) modelsMap[m] = [];
                    modelsMap[m].push(item);
                });

                // Build HTML per model
                const tableHTML = Object.entries(modelsMap).map(([modelName, modelItems]) => {
                    modelItems.sort((a, b) => (a.by_order ?? 9999) - (b.by_order ?? 9999));

                    const rowsHTML = modelItems.map((item, i) => `
                <tr draggable="true"
                    data-id="${item.id ?? ''}"
            ...
            `).join('');
                    ...
                    return `
                <h5 class="mt-3 mb-1 text-center">Model: ${modelName}</h5>
                <table class="table table-bordered table-sm text-center align-middle mb-4 tasks-table">
            ...
            `;
                ...
                Swal.fire({
                    ...
                    didOpen: () => enableDragAndDrop() // ✅ apply per table
                }).then(result => {
                    if (result.isConfirmed) {
                        // Save order per table

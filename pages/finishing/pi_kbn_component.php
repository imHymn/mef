    ...
    <script>
        ...
        function filterRows(data) {
            return data.filter(item =>
                item.status !== 'done' &&
                (item.section || '').toLowerCase().replace(/\s|-/g, '') === 'finishing'
            );
        }
        ...
        function renderRow(item) {
           ...
           data-mode="${hasTimeIn ? 'time-out' : 'time-in'}">
            ...
            row.innerHTML = `
                ...
                <span style="cursor: pointer; margin-right: 12px;" onclick="handleRefresh('${item.id}','finishing','${item.assembly_section}','finishing','stamping')">🔄</span>
            </td>

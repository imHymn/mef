...
<script>
  function getTasks(selectedItems) {
    ...
    // prioritize section_type, fallback to section
    const resolvedSection = first.section_type || first.section;

    scanQRCodeForUser({
      section: "FINISHING", // e.g., STAMPING, ASSEMBLY, FINISHING
      role: 'OPERATOR',
      userProductionLocation: first.user_production_location,
      onSuccess: ({
        user_id,
        full_name
      }) => {
        ...
        const itemsToInject = selectedItems.map(item => {
          const section = item.section_type || item.section;
          ...
          if (section === 'assembly') {
            ...
            return {
              ...
              material_no: item.material_no,
              material_description: item.material_description,
              rework: item.sub_component,
              replace: item.assembly_process,
              by_order: null,
              model: item.model,
              _section: 'finishing'
            };
        ...
        renderTasks(user_id, full_name, itemsToInject); // pass ALL items at once
      }
    });
  }
 ...
 <td class="text-center">${item.quantity ?? ''}</td>
  ...
  function renderTasks(user_id, full_name, selectedItems = []) {
    ...
    Promise.all([
        fetch(`api/assembly/getSpecificData_assigned?model=${encodeURIComponent(model)}&_=${Date.now()}`, {
          method: 'POST',
          headers: {
        ...
        fetch(`api/finishing/getSpecificData_assigned?model=${encodeURIComponent(model)}&_=${Date.now()}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
      ...
      .then(([assemblyRes, finishingRes, stampingRes]) => {
        const assemblyItems = (assemblyRes.items || []).filter(item => item.id && item.assembly_process)
          .map(item => ({
            ...item,
            _section: 'assembly'
          }))
        ...
        const finishingItems = (finishingRes.items || []).filter(item => item.id && item.material_no && item.material_description)
          .map(item => ({
            ...item,
            _section: 'finishing'
          }))
        ...
        const stampingItems = (stampingRes.items || []).filter(item => item.id && item.material_no && item.components_name)
          .map(item => ({
            ...item,
            _section: 'stamping'
          }))
        ...
        // Inject all selectedItems into correct lists
        selectedItems.forEach(item => {
          if (item._section === 'assembly') {
            assemblyItems.push(item);
          } else if (item._section === 'stamping') {
            stampingItems.push(item);
          } else {
            finishingItems.push(item);
          }
        });
      ...
      ${renderStampingTable(stampingItems)}
    `;
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ['assembly', 'finishing', 'stamping'].forEach(sec => {
              const rows = [...document.querySelectorAll(`#${sec}-tasks-table tbody tr`)];
              rows.forEach((tr, index) => {
                assign(tr.dataset.id, index + 1, full_name, sec);
              });
            });
          }
        });
    ...
    fetch(url, {
      ...
      .then(data => {
        if (data.success) {
          ...
          setTimeout(() => window.location.reload(), 2000); // give time for alert to show
        } else {

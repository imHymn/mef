    ...
    <script>
        ...
        function renderPaginatedTable(pageData) {
            ...
            pageData.forEach(item => {
                if (item.assembly_section !== 'FINISHING') return; // only show assembly section
            ...
            <td style="text-align: center;" class="sticky-col col-1">
                <span style="cursor: pointer; margin-right: 12px;" onclick="handleRefresh('${item.id}','finishing','${item.assembly_section}','finishing','assembly')">🔄</span>
            </td>
         ...
         <td style="text-align:center;"> ${highlightText( (item.person_incharge || '<i>Null</i>') + (item.by_order ? ` (${item.by_order})` : ''), currentFilterQuery )} </td>

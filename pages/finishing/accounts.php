...
<script>
    ...
    function renderTable(users) {
        ...
        users.forEach(user => {
            ...
            // Parse production and production_location safely
  ...
  <td>
    ...
    <button
      ...
      ${(userRole?.toLowerCase() === 'line leader' && (user.role ?? '').toLowerCase() === 'line leader') ? 'disabled' : ''}>
...
`;
    ...
    function loadAccounts() {
        fetch('api/accounts/getFinishingAccounts')
            .then(res => res.json())
            .then(data => {
                ...
                if (userRole === 'administrator') {
                    ...
                    const myProds = (userProduction || []).map(p => normalize(p));
                    ...
                    allUsers = data.filter(user => {
                        ...
                        return userProdArr.some(p => myProds.includes(normalize(p)));
                    });
                } else if (userRole === 'line leader') {
                    // Line leader: filter by production_location

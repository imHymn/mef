...
<div class="page-content">
                        ...
                        <table class="custom-hover table" style="table-layout: fixed; width: 100%;">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 8%; text-align: center;">Material Description</th>

                                    <th style="width: 5%; text-align: center;">Process</th>
                                    <th style="width: 7%; text-align: center;">Machine</th>
    ...
    <script>
        ...
        function filterRows(data) {
            const showOnly = ['l300 assy', 'hpi fender assy', 'mig welding'];
            return data.filter(item =>
                item.status !== 'done' &&
                showOnly.includes((item.section || '').toLowerCase())
            );
        }
        ...
        function renderTable(data, page = 1) {
            ...
            const filteredByRole = filtered.filter(item => {
                ...
                const allowed = ['l300 assy', 'hpi fender assy', 'finishing', 'mig welding'];
                return allowed.includes((item.section || '').trim().toLowerCase()) &&
        ...
        <td style="text-align: center;" class="sticky-col col-1">
            <span style="cursor: pointer; margin-right: 12px;" 
                  onclick="handleRefresh('${item.id}','assembly','${item.section}','reset','stamping','stamping')">🔄</span>
        </td>
